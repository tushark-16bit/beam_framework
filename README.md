...
as before. `PIPELINE` is an additional option that runs a fixed sequence as a single JAR
...
## PIPELINE — one call: submit, wait, run the report
...
Every declared datasource bypasses its own `COMPLETED` guard and re-downloads, superseding its
previous run's `DaRec` rows once complete, same as standalone `DATA_SOURCE_DOWNLOAD`; the report
itself needs no flag at all: it has no `COMPLETED` guard of its own and always re-runs fresh,
pipeline or standalone.

**Optional diagnostic:** `--processType=STATUS_CHECK` with the same `--reportName`/
`--reportSubprocess`/`--periodId` is available separately as a one-shot, non-blocking readiness
check — not needed for the flow above, useful for an ops dashboard or a manual look:
...
java -jar beam-runner-bundled.jar \
  ...
  --paramBqProject=my-gcp-project \
  --paramBqDataset=dw
# Exit 0 = ready, exit 75 = still pending (not an error), any other non-zero = terminal failure
```
...
`REPORT_PROCESSING`) or discovered by `DataSourceStatusChecker`'s in-process poll loop inside
`DATA_SOURCE_DOWNLOAD`/`PIPELINE` — it's thrown from the same `Main.main()` call that's still
running, blocked in that loop, so the same catch fires:
