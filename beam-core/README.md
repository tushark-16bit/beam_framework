...
| `model` | `ReportConfig`, `ReportDatasourceRef`, `ReportPreprocessingStep`, `ReportTransformStep`, `ReportOutputConfig`, `ReportEmailConfig` | Report configuration assembled from the report DB tables. `ReportEmailConfig` now also carries `fromAddress`/`encrypted` (from `email.from_address`/`email.encrypted`) |
...
|---|---|---|
| `DATA_SOURCE_DOWNLOAD` | `--processType=DATA_SOURCE_DOWNLOAD` | Fetch raw data from APIs/files/BQ; creates LOADING checkpoint, submits the job, returns immediately. **Does not wait** — this platform forbids `waitUntilFinish()`. BnC/checkpoint-finalize still runs post-pipeline, just inside the Beam worker (`PostDownloadFinalizeTransform`), not the driver. |
